...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    # - weights: weights of the regressor                        (D, K)   
    # - regularization: L2 regularization constant                  
    
    # Output:                                                         
    # - cost: cost of the regressor                                 
    # - grad: gradient of the regressor cost with respect to its  (D, K)  
    #        weights                                                                                             
    # - pred: label predictions of the regressor (you might find    
    #        np.argmax helpful)  
    
    prob = softmax(features.dot(weights)) # (N, K)
    ...
    gradprob = np.zeros_like(prob) # (N, K)
    gradprob += prob
    gradprob[range(N),labels]-=1
    grad = np.dot(features.T, gradprob)
    grad /= N
    grad += weights 

    ### YOUR CODE HERE: compute the gradients and predictions
    pred = np.argmax(prob, axis=1) #(N,)
