...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      self.L= tf.get_variable('Embedding', [len(self.wv), self.config.embed_size]) # initializer???
      window= tf.reshape( tf.nn.embedding_lookup(self.L, tf.reshape(self.input_placeholder, [-1])), 
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer1", initializer=xavier_weight_init()):
      self.W = tf.get_variable( "W", shape=(self.config.window_size*self.config.embed_size, self.config.hidden_size) )
      self.b1 = tf.get_variable("b1", shape=(self.config.hidden_size,))
      h = tf.nn.tanh(tf.matmul(window, self.W) + self.b1)
    with tf.variable_scope("Layer2", initializer=xavier_weight_init()):
      self.U = tf.get_variable( "U", shape=(self.config.hidden_size, self.config.label_size) )
      self.b2 = tf.get_variable("b2", shape=(self.config.label_size,))
      z = tf.matmul(h, self.U) + self.b2
    output = tf.nn.dropout(z, self.dropout_placeholder)
    ### END YOUR CODE
    return output 
  ...
  def add_loss_op(self, y):
    ...
    if self.config.l2:
      loss += 0.5*self.config.l2*tf.nn.l2_loss(self.W) + 0.5*self.config.l2*tf.nn.l2_loss(self.U)
    ### END YOUR CODE
    return loss
  ...
  def add_training_op(self, loss):
    ...
    train_op = optimizer.minimize(loss, global_step=global_step)    ### END YOUR CODE
    return train_op
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as session:
      ...
      for epoch in range(config.max_epochs):
        ...
        print(confusion(confusion, model.num_to_tag))
