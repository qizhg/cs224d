...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      self.L= tf.get_variable('Embedding', [len(self.vocab), self.config.embed_size]) # initializer???
      _inputs = tf.nn.embedding_lookup(self.L, self.input_placeholder)   # (batch_size, num_steps, embed_size)
      inputs  = [  tf.squeeze(x) for x in tf.split(1, self.config.num_steps, _inputs) ]
      ### END YOUR CODE
      return inputs
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("Projection"):
      ...
      self.b2 = tf.get_variable('b2', [len(self.vocab)])
      outputs = [  tf.matmul(rnn_output, self.U) + self.b2  for rnn_output in rnn_outputs ]
    ### END YOUR CODE
    return outputs
  ...
  def __init__(self, config):
    ...
    self.outputs = self.add_projection(self.rnn_outputs) # a list of (batch_size, len(vocab))
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("VanillaRNN"):
      ...
      rnn_outputs = []
      for x in inputs:
        state  = tf.nn.sigmoid(tf.matmul(state, self.H) + tf.matmul(x, self.I) + self.b1)
        rnn_outputs.append(state)
      self.final_state = rnn_outputs[-1]
    
    with tf.variable_scope("RNNOutputDropout"):
      rnn_outputs = [ tf.nn.dropout(h, self.dropout_placeholder) for h in rnn_outputs ]
    ### END YOUR CODE
    return rnn_outputs
